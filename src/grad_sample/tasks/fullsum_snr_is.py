    ...
    def __call__(self):
        ...
        for alpha in self.alpha_l :
            ...
            for state_idx in tqdm(self.eval_s):   
                ...
                print(fs_ng.shape)
                self.compute_S_F = nkjax.vmap_chunked(lambda s : _compute_S_F(s, self.vstate, self.chunk_size_jac, self.is_op, self.solver_fn, self.diag_shift), in_axes=0, chunk_size = self.chunk_size_resample)
                ...
                samples = self.vstate.sample_distribution(
                    log_Hpsi,
                    variables=Hpsi_vars, n_samples = self.Nsample * self.num_resample
                )
            ...
            jnp.savez(self.output_dir + f"/out_analysis_{alpha}_{self.diag_shift}.npz", alpha_out, self.diag_shift)
...
def _compute_S_F(samples, vstate, chunk_size, O, solver_fn, diag_shift):
    ...
    apply_fun, variables = O.get_log_importance(vstate)
    ...
    S = QGTJacobianDefaultConstructorIS(vstate._apply_fun,
        vstate.parameters,
        vstate.model_state,
        samples,
        apply_fun,
